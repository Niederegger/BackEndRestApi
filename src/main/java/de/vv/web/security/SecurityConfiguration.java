...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.httpBasic().and().authorizeRequests().antMatchers(HttpMethod.POST, "/account").hasRole("ADMIN")
				.antMatchers(HttpMethod.PUT, "/account/**").hasRole("ADMIN")
				.antMatchers(HttpMethod.PATCH, "/account/**").hasRole("ADMIN").and().csrf().disable();
	}
